...
# Redis布隆过滤器的优点
- 优点: 极省内存、查询/插入 O(k)，非常快、适合防缓存穿透
- 缺点：
  - 有误判率（假阳性）：说“存在”可能不存在，但说“不存在”一定不存在；误判率随数据量增加而上升，若初始化容量不足，会急剧恶化。
  - 不支持删除元素（原生）：
- 解决方案：
  - 误判导致业务影响：
    - 可接受场景：布隆过滤器作为“第一道筛子”，命中后回源 DB 或 Cache 二次校验（实际查询真实存储）。
    - 降低误判率：业务允许时设置更低的 error_rate（如 0.001），但会增大位数组长度。
  - 不支持删除：
    - 若必须删除：改用 计数布隆过滤器（Counting Bloom Filter），可将位改为计数器（Redis 实现较复杂，可用 Lua 脚本或基于 Hash 的计数）。
    - 改用 Cuckoo Filter（RedisBloom 支持 CF. 系列命令），支持删除且误判率更低。
    - 定期重建：将过期/无效 key 剔除后，重新构建新的布隆过滤器（双缓冲切换）。
# Cuckoo Filter vs. Bloom Filter
> Cuckoo Filter（布谷鸟过滤器）是一种替代布隆过滤器的概率性数据结构，主要优势是支持删除、空间效率更高（低误判率下）、查询性能更稳定，但也有一些代价。
...
| 特性 | 说明 |
| --- | --- |
| 支持删除 | 布隆过滤器原生不支持删除（除非用计数版，但开销大）。Cuckoo Filter 可通过 `CF.DELETE` 安全删除元素，且不会误删其他元素。 |
| 空间效率更高 | 在相同误判率（如 < 3%）下，Cuckoo Filter 通常比布隆过滤器占用更少内存。例如：误判率 0.1% 时，Cuckoo Filter 内存比布隆过滤器低约 30~50%。 |
| 查询性能稳定 | 布隆过滤器需要 k 次哈希 + k 次内存访问（k 通常 7~14）。Cuckoo Filter 最多两次哈希 + 两次内存访问（因每个元素只有一个或两个候选位置），CPU 缓存更友好。 |
| 误判率可预测且不随容量增长而恶化 | 布隆过滤器在超过容量后误判率急剧上升。Cuckoo Filter 接近满负载时插入可能失败（需扩容或踢出），但误判率保持稳定。 |
| 支持动态扩容 | RedisBloom 的 `CF.RESERVE` 支持 `EXPANSION`，可动态扩展容量，不会像布隆过滤器那样超限后性能崩溃。 |

## 二、Cuckoo Filter 的坏处

| 特性 | 说明 |
| --- | --- |
| 插入可能失败 | 当哈希表负载过高（>95%），插入可能因踢出循环而失败。需要重试、扩容或重新哈希。布隆过滤器永远不会拒绝插入（只可能增加误判率）。 |
| 删除非 100% 安全（假阴性风险） | 若插入相同元素多次，删除一次可能导致该元素实际上还存在，但后续查询可能返回不存在（假阴性）。布隆过滤器无删除，无此问题。 |
| 实现复杂度高 | 布隆过滤器只需位数组 + 几个哈希函数。Cuckoo Filter 需要管理桶、指纹、踢出逻辑，代码更复杂。 |
| 对哈希函数质量要求更高 | 需要两个独立的哈希函数计算候选位置，且指纹计算要均匀。不良哈希会导致频繁踢出甚至插入失败。 |
| Redis 模块命令较多 | RedisBloom 中的 Cuckoo Filter 命令（`CF.ADD`、`CF.DEL`）在集群环境下需要小心处理跨槽问题（因为 key 是同一个过滤器）。 |
| 不支持计数 | 与计数布隆过滤器不同，Cuckoo Filter 无法知道一个元素插入的次数，删除一次就移除一个指纹。 |
